...
function render_column(
    d::AccretionDisk, geodesics::AbstractArray, height::Int, fov_index::Int, x::Int
)
    ...
    for y in 1:(height - 1)
        ...
        i = convert(Int, round(1 + r / fov_index * n))
        if i < n
...
function renderdisk(
    ...
    for x in (-mid):(mid - 1)
        col = render_column(d, geodesics, height, fov_index, x)
